...
CookieStand.prototype.randomRate = function(){
  for(var i = 0; i < (this.closeTime - this.openTime); i++){
    this.cookiesSoldArray.push(Math.floor(this.aveCookies * (Math.random() * (this.max - this.min + 1) + this.min)));
  };
  ...
  console.log('Dana',this.cookiesSoldArray);
...
CookieStand.prototype.createHourlyRateTable = function(){
  ...
  var locCSArray = this.cookiesSoldArray.slice(0);
  // this.cookiesSoldArray.unshift(this.name);
...
function handleLocationSubmit(event) {
  ...
  if(CookieStand.allLocationsRef.indexOf(name) > -1){
    ...
    CookieStand.all[CookieStand.allLocationsRef.indexOf(name)].cookiesSoldArray = [];
...
var sumColumns = function(){
  CookieStand.totalHourArray = [];
  for(var j = -1; j < CookieStand.all[0].cookiesSoldArray.length ; j++){
    var hourTotal = 0;
    for(var k in CookieStand.all){
      hourTotal = hourTotal + CookieStand.all[k].cookiesSoldArray[j];
    };
